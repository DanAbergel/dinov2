...
Reads runs/v2/<run>/training_metrics.json (DINOv2 writes one JSON record per
...
Usage (on Moriah, in an env with matplotlib — e.g. torch_env):
    ...
    python tasks/v2/plot_losses.py
    -> writes /sci/labs/arieljaffe/dan.abergel1/runs/v2/dino_losses.png
"""
...
def load(run):
    ...
    path = f"{LAB}/runs/v2/{run}/training_metrics.json"
    txt = open(path).read()
    records = []
...
def main():
    ...
    fig.tight_layout()
    out = f"{LAB}/runs/v2/dino_losses.png"
